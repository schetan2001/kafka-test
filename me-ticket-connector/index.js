...
const ZOHO_REFRESH_TOKEN = process.env.ZOHO_REFRESH_TOKEN ||
  "1000.de9f6a55b1bc15f3a7054cae27cbe897.efd51e07c78d8875ec84797452d45a26";
const ZOHO_CLIENT_ID = process.env.ZOHO_CLIENT_ID || "1000.JARQGYYRTK7II3HNYA24RJRTA3JYUU";
const ZOHO_CLIENT_SECRET = process.env.ZOHO_CLIENT_SECRET || "84fdafbd326346583d03075e0047368b594f8240da";

const TOKEN_API_URL = process.env.TOKEN_API_URL ||
  `https://accounts.zoho.in/oauth/v2/token?refresh_token=${ZOHO_REFRESH_TOKEN}&grant_type=refresh_token&client_id=${ZOHO_CLIENT_ID}&client_secret=${ZOHO_CLIENT_SECRET}&redirect_uri=https%3A%2F%2Fsdpondemand.manageengine.in%2Fhome%2F&scope=SDPOnDemand.requests.CREATE,SDPOnDemand.requests.UPDATE`;

const TOKEN_HEADERS = {
  Cookie: process.env.ZOHO_TOKEN_COOKIE ||
...
async function handleKafkaMessage(payload) {
  ...
  if (!systemId || !dtcId || !status) {
    ...
    const getCategoryFromSeverity = (sev) => {
  ...
  const upperCaseSeverity = severity?.toUpperCase() || 'MEDIUM';
    ...
    if (isCloseStatus && ticketExists) {
      ...
      const resolutionPayload = {
        request: {
          ...
          udf_fields: {
            date_dtc_closed_timestamp: { value: String(clearedAtEpoch) },
