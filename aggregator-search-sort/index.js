const express = require("express");
const axios = require("axios");
const dotenv = require("dotenv");
// const cors = require("cors");

dotenv.config();

const app = express();
app.use(express.json());

const { sortData, filterData } = require("./utility");

// If you need CORS uncomment the following block:
/*
const corsOptions = {
  origin: [
    "https://tap-sit.royalenfield.com",
    "http://localhost:3000",
    "http://localhost:3001",
  ],
  methods: "GET,HEAD,PUT,PATCH,POST,DELETE",
  credentials: true,
  allowedHeaders: ["Content-Type", "Authorization", "x-api-key"],
};
app.use(cors(corsOptions));
*/

const INGRESS_API_KEY = process.env.API_KEY;

const API_CONFIGS = [
  {
    curl: process.env.CURL1,
    keys: process.env.KEYS1,
    path: process.env.PATH1,
    name: "service1",
    enableSearch: process.env.ENABLE_SEARCH1 === "1",
    enableSort: process.env.ENABLE_SORT1 === "1",
  },
  {
    curl: process.env.CURL2,
    keys: process.env.KEYS2,
    path: process.env.PATH2,
    name: "service2",
    enableSearch: process.env.ENABLE_SEARCH2 === "1",
    enableSort: process.env.ENABLE_SORT2 === "1",
  },
];

async function parseCurl(curl) {
  try {
    const curlconverter = await import("curlconverter");
    const parsed = curlconverter.toJsonObject(curl);
    const { raw_url, headers } = parsed;
    return { url: raw_url, headers };
  } catch (err) {
    console.error("cURL Parse Error on:", curl);
    throw new Error("Invalid cURL string");
  }
}

function getNestedValue(obj, path) {
  if (!path) return obj;
  return path.split(".").reduce((acc, part) => acc && acc[part], obj);
}

app.use((req, res, next) => {
  const clientKey = req.headers["x-api-key"];
  if (!clientKey || clientKey !== INGRESS_API_KEY) {
    return res.status(401).json({ error: "Unauthorized" });
  }
  next();
});

app.get("/aggregate", async (req, res) => {
  try {
    const validConfigs = API_CONFIGS.filter(
      (config) => config.curl && config.keys && config.path
    );

    if (validConfigs.length === 0) {
      return res.status(500).json({
        error: "At least one CURL, KEYS, and PATH trio must be provided.",
      });
    }

    const configMap = validConfigs.reduce((map, config) => {
      map[config.name] = config;
      return map;
    }, {});

    const requests = validConfigs.map(async (config) => {
      try {
        const { url: baseUrl, headers } = await parseCurl(config.curl);
        const dynamicUrl = new URL(baseUrl);
        for (const key in req.query) {
          dynamicUrl.searchParams.set(key, req.query[key]);
        }

        const response = await axios.get(dynamicUrl.toString(), { headers });
        return { data: response.data, config };
      } catch (e) {
        console.error(`Error fetching data for ${config.name}:`, e.message);
        return { error: e.message, config };
      }
    });

    const responses = await Promise.all(requests);

    let results = {};
    responses.forEach((response) => {
      const { data, config } = response;
      if (response.error) {
        results[config.name] = { error: response.error };
        return;
      }
      const pathSegments = config.path.split(" - ").map((p) => p.trim());
      const keyGroups = config.keys.split(" - ").map((k) => k.trim());
      if (pathSegments.length !== keyGroups.length) {
        results[config.name] = {
          error: "Mismatched number of path segments and key groups.",
        };
        return;
      }

      const extractedData = {};
      pathSegments.forEach((pathSegment, index) => {
        const keysToExtract = keyGroups[index].split(",").map((k) => k.trim());
        const dataToExtract = getNestedValue(data, pathSegment);
        if (Array.isArray(dataToExtract)) {
          extractedData[pathSegment] = dataToExtract.map((item) => {
            const extracted = {};
            keysToExtract.forEach((key) => {
              if (item.hasOwnProperty(key)) {
                extracted[key] = item[key];
              }
            });
            return extracted;
          });
        } else if (
          typeof dataToExtract === "object" &&
          dataToExtract !== null
        ) {
          const extracted = {};
          keysToExtract.forEach((key) => {
            if (dataToExtract.hasOwnProperty(key)) {
              extracted[key] = dataToExtract[key];
            }
          });
          extractedData[pathSegment] = extracted;
        } else {
          console.warn(
            `Path '${pathSegment}' did not lead to a valid object or array for ${config.name}.`
          );
          extractedData[pathSegment] = dataToExtract;
        }
      });
      results[config.name] = extractedData;
    });

    const searchQuery = req.query.search;
    const sortField = req.query.sort;

    for (const serviceName in results) {
      const serviceConfig = configMap[serviceName];

      if (results[serviceName].error || !serviceConfig) continue;

      // 1. Get all path and key segment groups for the current service configuration
      const allPathSegments = serviceConfig.path
        .split(" - ")
        .map((p) => p.trim());
      const allKeyGroups = serviceConfig.keys.split(" - ").map((k) => k.trim());

      // Safety check: ensure paths and keys still align
      if (allPathSegments.length !== allKeyGroups.length) continue;

      for (const dataKey in results[serviceName]) {
        // dataKey is the pathSegment (e.g., 'responseData.vehicleDetails')
        let dataArray = results[serviceName][dataKey];

        if (Array.isArray(dataArray)) {
          // 2. Find the index of the current dataKey (pathSegment) in the configuration
          const pathIndex = allPathSegments.indexOf(dataKey);

          // 3. If found, use the corresponding key group for search/sort fields
          if (pathIndex !== -1) {
            const keyGroup = allKeyGroups[pathIndex] || "";

            const serviceSearchFields = keyGroup
              .split(",")
              .map((k) => k.trim())
              .filter((k) => k);

            if (
              serviceConfig.enableSearch &&
              searchQuery &&
              serviceSearchFields.length > 0
            ) {
              dataArray = filterData(
                dataArray,
                searchQuery,
                serviceSearchFields
              );
            }

            if (serviceConfig.enableSort && sortField) {
              const direction = req.query.direction || "asc";
              dataArray = sortData(dataArray, sortField, direction);
            }
          } // end pathIndex check
          results[serviceName][dataKey] = dataArray;
        }
      }
    }

    res.json(results);
  } catch (err) {
    console.error("Internal error:", err);
    res.status(500).json({ error: "Internal Server Error" });
  }
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`Ingress listening on port ${PORT}`);
});
